...
@interface HSQStoreClassListHeadView ()
...
@implementation HSQStoreClassListHeadView
...
- (void)AddRightButton{
    ...
    [Right_Btn setImage:KImageName(@"C9EDA7DE-0473-4C43-939C-0572B2084BC9") forState:(UIControlStateNormal)];
    
    Right_Btn.frame = CGRectMake(KScreenWidth - KViewHeight, 0, KViewHeight, KViewHeight);
...
- (void)SetUpScrollerView{
    ...
    UIScrollView *scrollerView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth - KViewHeight, KViewHeight)];
...
- (void)setStoreClassList:(NSMutableArray *)storeClassList{
    
    _storeClassList = storeClassList;
    
    [self AddTitleButtonWithstoreClassList:storeClassList];
    
    [self CreatMenuViewWithstoreClassList:storeClassList];
}

/**
 * @brief 添加标题按钮
 */
- (void)AddTitleButtonWithstoreClassList:(NSMutableArray *)title_array{
    
    //    self.scrollerView.contentSize = CGSizeMake(title_array.count * self.scrollerView.size.width / KShowCount , KViewHeight);
    ...
    for (NSInteger i = 0; i < title_array.count; i++) {
        ...
        HSQStoreClassListModel *model = title_array[i];
        
        UIButton *title_btn = [UIButton buttonWithType:(UIButtonTypeSystem)];
        
        [title_btn setTitle:model.name forState:(UIControlStateNormal)];
        ...
        CGSize Title_Size = [NSString SizeOfTheText:model.name font:[UIFont systemFontOfSize:KLabelFont(14.0, 12.0)] MaxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
        ...
        {
            HSQStoreClassListModel *model = title_array[i - 1];
        ...
        HSQStoreClassListModel *Max_model = title_array.lastObject;
        
        CGSize Max_Title_Size = [NSString SizeOfTheText:Max_model.name font:[UIFont systemFontOfSize:KLabelFont(14.0, 12.0)] MaxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    ...
    self.scrollerView.contentSize = CGSizeMake(contentW, KViewHeight);
 ...
 * @brief 创建下拉菜单的视图
 */
- (void)CreatMenuViewWithstoreClassList:(NSMutableArray *)title_array{
    
    CGFloat bgView_Y = KSafeTopeHeight + KViewHeight;
...
- (void)MenuTitleBtnClickAction:(UIButton *)sender{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(StoreClassBtnClickAction:)]) {

        [self.delegate StoreClassBtnClickAction:sender];
    }
}
