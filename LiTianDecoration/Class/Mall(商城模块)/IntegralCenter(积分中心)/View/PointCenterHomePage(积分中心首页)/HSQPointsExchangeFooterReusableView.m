...
-(void)setMemberGradeList:(NSMutableArray *)memberGradeList{
    ...
    // 内部的子标签
    CGFloat width = titlesView.mj_w / memberGradeList.count;
    
    CGFloat height = titlesView.mj_h;
    
    for (NSInteger i = 0; i < memberGradeList.count; i++) {
        ...
        HSQMembershipListModel *model = memberGradeList[i];
        
        [button setTitle:model.gradeName forState:UIControlStateNormal];
