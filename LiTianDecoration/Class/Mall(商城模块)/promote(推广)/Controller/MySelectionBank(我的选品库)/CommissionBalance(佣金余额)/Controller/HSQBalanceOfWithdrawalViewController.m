//
//  HSQBalanceOfWithdrawalViewController.m
//  LiTianDecoration
//
...
#import "HSQApplyCommissionWithdrawalViewController.h"  // 佣金提现
...
@interface HSQBalanceOfWithdrawalViewController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) UIButton *ApplyWithdrawals_Btn; // 申请提现
...
@implementation HSQBalanceOfWithdrawalViewController
...
- (HSQNoDataView *)noDataView{
    ...
    if (!_noDataView) {
        ...
        _noDataView = [[HSQNoDataView alloc] initWithTitle:@"亲，还没有相关的提现数据额" imageName:@"WaitingForView" height:50 TopMargin:0];
    }
    return _noDataView;
}
...
- (void)viewDidLoad {
    ...
    // 申请提现
    [self AddApplyWithdrawals_Btn];
    
    // 监听提现是否成功
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(WithdrawalSuccess:) name:@"WithdrawalSuccess" object:nil];
 ...
 * @brief 申请提现按钮
 */
- (void)AddApplyWithdrawals_Btn{
    
    UIButton *ApplyWithdrawals_Btn = [UIButton buttonWithType:(UIButtonTypeSystem)];
    
    [ApplyWithdrawals_Btn setTitle:@"申请提现" forState:(UIControlStateNormal)];
    
    [ApplyWithdrawals_Btn setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
    
    [ApplyWithdrawals_Btn setBackgroundImage:[UIImage ImageWithColor:RGB(238, 58, 68)] forState:(UIControlStateNormal)];
    
    [ApplyWithdrawals_Btn addTarget:self action:@selector(ApplyWithdrawals_BtnClickAction:) forControlEvents:(UIControlEventTouchUpInside)];
    
    CGFloat ApplyWithdrawals_BtnH = KScreenHeight - KSafeTopeHeight - KSafeBottomHeight - 100 - 44;
    
    ApplyWithdrawals_Btn.frame = CGRectMake(0, ApplyWithdrawals_BtnH, KScreenWidth, 44);
    
    [self.view addSubview:ApplyWithdrawals_Btn];
    
    self.ApplyWithdrawals_Btn = ApplyWithdrawals_Btn;
}

/**
 * @brief 申请提现按钮点击
 */
- (void)ApplyWithdrawals_BtnClickAction:(UIButton *)sender{
    
    HSQApplyCommissionWithdrawalViewController *ApplyCommissionWithdrawalVC = [[HSQApplyCommissionWithdrawalViewController alloc] init];
    
    [self.navigationController pushViewController:ApplyCommissionWithdrawalVC animated:YES];
}

/**
 * @brief 监听提现是否成功
 */
- (void)WithdrawalSuccess:(NSNotification *)notif{
...
- (void)LoadNewPromotionCommissionWithdrawalListData{
    ...
    [requestTool.manger POST:UrlAdress(KCommissionWithdrawalListUrl) parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
