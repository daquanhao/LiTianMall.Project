...
@interface HSQSubmitCouperListView ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HSQSubmitCouperListView
...
+ (instancetype)initSubmitCouperListView{
    ...
    HSQSubmitCouperListView *publicView = [[HSQSubmitCouperListView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, KViewHeight)];
...
-(void)SetValueDataWithArray:(NSArray *)data_Array Select_Index:(NSString *)index{
    ...
    for (NSInteger i = 0; i < data_Array.count; i++) {
        ...
        NSString *couperid = [NSString stringWithFormat:@"%@",diction[@"conformId"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0)
    {
        cell.ShengMonery_Label.text = [NSString stringWithFormat:@"%@",@"不使用优惠券"];
    ...
    else
    {
        ...
        {
            NSDictionary *diction = self.dataSource[indexPath.row-1];
            
            cell.ShengMonery_Label.text = [NSString stringWithFormat:@"%@",diction[@"shortRule"]];
            
            cell.ActivityName_Label.text = [NSString stringWithFormat:@"满%@元可用",diction[@"limitAmount"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.SelectCouperDataBlock)
    {
        self.SelectCouperDataBlock(indexPath);
    }
