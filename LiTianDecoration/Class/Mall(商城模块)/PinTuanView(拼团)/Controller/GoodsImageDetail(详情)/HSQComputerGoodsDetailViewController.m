//
//  HSQComputerGoodsDetailViewController.m
//  LiTianDecoration
//
...
@interface HSQComputerGoodsDetailViewController ()<WKNavigationDelegate>
...
@implementation HSQComputerGoodsDetailViewController

- (void)viewDidLoad {
    
    [super viewDidLoad];
    
     self.navigationItem.title = @"电脑版商品详情";
...
- (void)RequestUserRegisterDataFromeServer{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",UrlAdress(KComputerGoodsDetailUrl),self.commonId];
    ...
    [requestTool.manger GET:url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        {
            [self.webView loadHTMLString:responseObject[@"datas"][@"goodsBody"] baseURL:nil];
        }
