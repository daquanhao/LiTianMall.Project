//
//  HSQLeftCouponsViewController.m
//  LiTianDecoration
//
...
@interface HSQLeftCouponsViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HSQLeftCouponsViewController
...
- (HSQNoDataView *)noDataView{
    ...
    if (!_noDataView) {
        ...
        _noDataView = [[HSQNoDataView alloc] initWithTitle:@"亲，还没有相关的优惠券额" imageName:@"WaitingForView" height:50 TopMargin:0];
    }
    return _noDataView;
}
...
- (void)viewDidLoad {
    ...
    // 监听卡密码红包领取成功的消息
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(VoucherWasSuccessfullyReceivedNotif:) name:@"VoucherWasSuccessfullyReceivedNotif" object:nil];
}
...
- (void)CreatTableView{
    ...
    [tableView registerClass:[HSQMemberCouponListCell class] forCellReuseIdentifier:@"HSQMemberCouponListCell"];
...
- (void)LoadNewWaitPayMoneryOrderListData{
    ...
    [requestTool.manger POST:UrlAdress(KMembershipCouponListUrl) parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        {
            self.dataSource = [HSQVoucherListModel mj_objectArrayWithKeyValuesArray:responseObject[@"datas"][@"voucherList"]];
...
- (void)LoadMoreWaitPayMoneryOrderData{
    ...
    [requestTool.manger POST:UrlAdress(KMembershipCouponListUrl) parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([responseObject[@"code"] integerValue] == 200)
        {
            NSArray *array = [HSQVoucherListModel mj_objectArrayWithKeyValuesArray:responseObject[@"datas"][@"voucherList"]];
...
- (void)ScreenRedEnvelopes{
    ...
    for (HSQVoucherListModel *model in self.dataSource) {
        ...
        if (model.voucherState.integerValue == 1 || model.voucherState.integerValue == 2)
        {
 ...
 * @brief 监听卡密码红包领取成功
 */
- (void)VoucherWasSuccessfullyReceivedNotif:(NSNotification *)notification{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    {
        HeaderView.placher_Label.text = @"已失效优惠券";
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HSQVoucherListModel *model = array[indexPath.row];
    
    return [self.tableView cellHeightForIndexPath:indexPath model:model keyPath:@"model" cellClass:[HSQMemberCouponListCell class] contentViewWidth:KScreenWidth] + 10;
    
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    HSQMemberCouponListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HSQMemberCouponListCell" forIndexPath:indexPath];
