//
//  HSQEvaluateOrderListViewController.m
//  LiTianDecoration
//
...
#import "HSQEvaluateOrderListViewController.h"
...
@interface HSQEvaluateOrderListViewController ()<UITableViewDelegate,UITableViewDataSource,HSQOrderGoodsListCellDelegate>
...
@implementation HSQEvaluateOrderListViewController
...
- (void)viewDidLoad {
    ...
    // 订单列表--确认收货的数据
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ReLoadNewDataFromeServer:) name:@"OrderListQRShouHuoSuccessNotif" object:nil];
}
        ...
        for (NSDictionary *dict in responseObject[@"datas"][@"ordersPayVoList"]){
            ...
            for (NSInteger i = 0; i < [dict[@"ordersVoList"] count] ; i++) {
                ...
                for (NSDictionary *ThirdDiction in ModelDiction[@"ordersGoodsVoList"]) {
                    ...
                    ThirdModel.Select_Arrays = [NSMutableArray array];
...
- (void)DeleteOrderButtonClickAction:(UIButton *)sender{
    ...
    if (secondModel.ordersState.integerValue == 40)
    {
...
- (void)OrderCancelBtnClickAction:(UIButton *)sender{
    ...
    params[@"ordersId"] = secondModel.ordersId;
...
/**
 * @brief 接收待收货订单列表的，确认收货的消息
 */
- (void)ReLoadNewDataFromeServer:(NSNotification *)notif{
