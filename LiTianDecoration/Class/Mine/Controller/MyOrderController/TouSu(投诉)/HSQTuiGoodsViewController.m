...
#import "HSQTuiGoodsViewController.h"
#import "HSQAccountTool.h"
#import "HSQChooseTuiKuanReasonView.h"
#import "HSQReturnGoodsViewController.h"  // 退货列表

@interface HSQTuiGoodsViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITextFieldDelegate>
...
@property (weak, nonatomic) IBOutlet UILabel *TuiHuoReason_Label; // 退货原因

@property (weak, nonatomic) IBOutlet UILabel *LeftTuiKuanMonery_Label; // 左边的金额

@property (weak, nonatomic) IBOutlet UILabel *RightTuiKuanMonery_Label; // 右边的金额

@property (weak, nonatomic) IBOutlet UILabel *RightTuiHuoCount_Label;
...
@implementation HSQTuiGoodsViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"商品退货";
    ...
    // 请求退货商品的数据
    [self RequestDataOnReturnedGoods];
}

/**
 * @brief 请求退货商品的数据
 */
- (void)RequestDataOnReturnedGoods{
    ...
    [requestTool.manger POST:UrlAdress(KTuiHuoSehnQingUrl) parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)SetValueViewWithDiction:(NSDictionary *)diction{
    ...
    // 退款的金额
    self.LeftTuiKuanMonery_Label.text = [NSString stringWithFormat:@"¥%@",diction[@"ordersGoodsVo"][@"goodsPayAmount"]];
    ...
    self.RightTuiKuanMonery_Label.attributedText = attribe;
    
    // 退货的数量
    self.Count_Label.text = [NSString stringWithFormat:@"%@",diction[@"ordersGoodsVo"][@"buyNum"]];
    
    // 右边退款的金额
    NSString *buyNum = [NSString stringWithFormat:@"最多%@%@",diction[@"ordersGoodsVo"][@"buyNum"],diction[@"ordersGoodsVo"][@"unitName"]];
    NSString *buyNum02 = [NSString stringWithFormat:@"%@\n最多可退数量",buyNum];
    NSMutableAttributedString *attribe02 = [[NSMutableAttributedString alloc] initWithString:buyNum02];
    [attribe02 addAttribute: NSForegroundColorAttributeName value:[UIColor greenColor] range:NSMakeRange(0, buyNum.length)];
    self.RightTuiHuoCount_Label.attributedText = attribe02;
    
    // 退款的原因
    self.TuiHuoReason_Label.text = [NSString stringWithFormat:@"%@",diction[@"refundReasonList"][0][@"reasonInfo"]];
 ...
 * @brief 选择退货原因
 */
- (IBAction)ChooseTuiHuoReasonBtnClickAction:(UIButton *)sender {
    ...
    coupListView.SelectReasonDataBlock = ^(NSIndexPath *indexPath) {
        ...
        self.TuiHuoReason_Label.text = [NSString stringWithFormat:@"%@",diction[@"reasonInfo"]];
 ...
 * @brief 选择退货凭证
 */
- (IBAction)ChooseTuiHUoPingZhengBtnClickAction:(UIButton *)sender {
    
    self.Index = sender.tag - 405;
...
- (IBAction)SubmitButtonClickAction:(UIButton *)sender {
    ...
    if (self.ShuoMing_TextField.text.length == 0)
    {
        [[HSQProgressHUDManger Manger] ShowProgressHUDPromptText:@"请填写退货说明" SupView:self.view];
    }
    else
    {
        UIImage *imageV = [UIImage imageNamed:@"123"];
...
- (void)SubmitDataToServer{
    ...
    params[@"buyerMessage"] = self.ShuoMing_TextField.text;
    params[@"ordersGoodsId"] = self.Goods_Id;
    params[@"reasonId"] = self.reasonId;
    params[@"goodsNum"] = self.dataDiction[@"ordersGoodsVo"][@"buyNum"];
    ...
    [requestTool.manger POST:UrlAdress(KTuiHuoShenQingSave)  parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([responseObject[@"code"] integerValue] == 200)  // 操作成功以后，进入退货列表
        {
            HSQReturnGoodsViewController *moneryVC = [[HSQReturnGoodsViewController alloc] init];
            
            moneryVC.index_Number = @"200";
