...
@interface HSQWebViewController ()<WKNavigationDelegate>
...
@implementation HSQWebViewController
...
- (void)RequestUserRegisterDataFromeServer{
    ...
    [RequestTool.manger GET:self.Url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        if ([[responseObject allKeys] containsObject:@"datas"])
        {
            self.navigationItem.title = [NSString stringWithFormat:@"%@",responseObject[@"datas"][@"title"]];
            
            [self.webView loadHTMLString:responseObject[@"datas"][@"content"] baseURL:nil];
        }
